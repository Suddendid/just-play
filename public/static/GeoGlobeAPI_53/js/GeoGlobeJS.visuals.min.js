...
255+0.5)*4+2]+","+a.gradientImageData[3]/255+")":d.color;else{if(GeoGlobe.Util.getType(d.color)!=="object"){console.error("\u975e\u6241\u5e73\u6837\u5f0f\u4e0b\u8bf7\u4f7f\u7528\u6e10\u53d8\u8272\uff01");return}var c=0,n;for(n in d.color)+n>c&&(c=+n);this.color=d.color[c]}this.rgbColor=GeoGlobe.Util.getRgbColor(this.color);this.tmpColor=this.color;this.alpha=this.color.indexOf("a(")!==-1?+this.color.split(",")[3].split(")")[0]:1;if(b.valueFilter)for(n=0;n<b.valueFilter.length;n++)if(this.value===
b.valueFilter[n])this.tmpColor=this.rgbColor=b.filterColor[n],this.size=b.filterSize[n],this.increment=b.filterIncrement[n];if(b.threshold)for(n=0;n<b.threshold.length;n++)if(b.threshold[n+1]&&b.threshold[n]<this.value&&this.value<b.threshold[n+1])this.tmpColor=this.rgbColor=b.thresholdColor[n],this.size=b.thresholdSize[n],this.increment=b.thresholdIncrement[n];switch(b.symbol){case "fire":this.fires=[];for(n=0;n<150;n++)this.fires.push(new a.Fire(this.size));break;case "water":this.waveLength=0.1;
...
c:this.value},b.formatter).split("<br/>"),n=m.length,o=0;o<n;o++){var v=c.measureText(m[o]).width;v>j&&(j=v);c.fillText(m[o],this.x,this.y-((n-1)/2-o)*(this.radius*2+5))}else j=c.measureText("\uffe5"+this.value).width;c.rect(this.x-j/2,this.y-l/2,j,l)}break;case "fire":c.rect(this.x-this.size/2,this.y-this.size/2,this.size,this.size);break;case "icon":c.rect(this.x-b.symbolWidth/2,this.y-b.symbolHeight/2,b.symbolWidth,b.symbolHeight)}if(c.isPointInPath(d,h))a.hoveredMarkPoint=this};this.MarkPoint.prototype.draw1=
...
j;return}break;case "pie":for(h=0;h<1;h++){var s=this.parts[h].tCentrePoint,q=this.parts[h].oUnderPoints,r=this.parts[h].oTopPoints,t=this.parts[h].iUnderPoints,x=this.parts[h].iTopPoints,A=!1;this.parts[h].size/2>g&&(A=!0);var B=Math.round(25),z=[],y=[],w=[];l=[];for(j=0;j<this.parts[h].sectors.length;j++)z.push(this.parts[h].sectors[j].angle),y.push(this.parts[h].sectors[j].startIndex),w.push(Math.round(this.parts[h].sectors[j].endIndex*this.energy)),l.push(b[j]?b[j]:m.color);for(j=z.length-1;j>=
0;j--)if(z[j]>0){c.beginPath();if(y[j]>B){c.moveTo(r[w[j]].get2d().x,r[w[j]].get2d().y);for(l=w[j]-1;l>=y[j];l--)c.lineTo(r[l].get2d().x,r[l].get2d().y);if(A){c.lineTo(x[y[j]].get2d().x,x[y[j]].get2d().y);for(l=y[j];l<=w[j];l++)c.lineTo(t[l].get2d().x,t[l].get2d().y);c.lineTo(x[w[j]].get2d().x,x[w[j]].get2d().y)}else c.lineTo(s.get2d().x,s.get2d().y)}else if(w[j]<B){c.moveTo(q[w[j]].get2d().x,q[w[j]].get2d().y);for(l=w[j]-1;l>=y[j];l--)c.lineTo(q[l].get2d().x,q[l].get2d().y);c.lineTo(r[y[j]].get2d().x,
r[y[j]].get2d().y);if(A)for(l=y[j];l<=w[j];l++)c.lineTo(x[l].get2d().x,x[l].get2d().y);else c.lineTo(s.get2d().x,s.get2d().y);c.lineTo(r[w[j]].get2d().x,r[w[j]].get2d().y)}else{c.moveTo(r[w[j]].get2d().x,r[w[j]].get2d().y);for(l=w[j]-1;l>=B;l--)c.lineTo(r[l].get2d().x,r[l].get2d().y);for(l=B;l>=y[j];l--)c.lineTo(q[l].get2d().x,q[l].get2d().y);c.lineTo(r[y[j]].get2d().x,r[y[j]].get2d().y);if(A){for(l=y[j];l<=B;l++)c.lineTo(x[l].get2d().x,x[l].get2d().y);for(l=B;l<=w[j];l++)c.lineTo(t[l].get2d().x,
t[l].get2d().y)}else c.lineTo(s.get2d().x,s.get2d().y)}if(c.isPointInPath(e,f)){a.hoveredMarkPoint=this;this.hActive=h;this.vActive=j;return}}}}};this.MarkPoint.prototype.draw=function(c){var e=a._rendererOptions.markPoint.slope;this.update();var f,k,p;p=!1;var u;switch(d){case "column":for(f=0;f<this.parts.length;f++)for(k=0;k<this.parts[f].cubes.length;k++)if(this.parts[f].cubes[k].height>0&&(p=!0,u=b[k]?b[k]:m.color,a.hoveredMarkPoint===this&&f===this.hActive&&k===this.vActive&&(u=GeoGlobe.Util.getShadeColor(u,
20)),c.globalAlpha=l,c.globalCompositeOperation="source-over",c.shadowBlur=0,c.shadowOffsetX=0,c.shadowOffsetY=0,c.beginPath(),c.moveTo(this.parts[f].cubes[k].vertexs[0][0],this.parts[f].cubes[k].vertexs[0][1]),c.lineTo(this.parts[f].cubes[k].vertexs[0][0],this.parts[f].cubes[k].vertexs[0][1]-this.parts[f].cubes[k].height*this.energy),c.lineTo(this.parts[f].cubes[k].vertexs[3][0],this.parts[f].cubes[k].vertexs[3][1]-this.parts[f].cubes[k].height*this.energy),c.lineTo(this.parts[f].cubes[k].vertexs[3][0],
this.parts[f].cubes[k].vertexs[3][1]),c.closePath(),c.fillStyle=u,c.fill(),h!==0))c.beginPath(),c.moveTo(this.parts[f].cubes[k].vertexs[0][0],this.parts[f].cubes[k].vertexs[0][1]),c.lineTo(this.parts[f].cubes[k].vertexs[1][0],this.parts[f].cubes[k].vertexs[1][1]),c.lineTo(this.parts[f].cubes[k].vertexs[1][0],this.parts[f].cubes[k].vertexs[1][1]-this.parts[f].cubes[k].height*this.energy),c.lineTo(this.parts[f].cubes[k].vertexs[0][0],this.parts[f].cubes[k].vertexs[0][1]-this.parts[f].cubes[k].height*
this.energy),c.closePath(),c.fillStyle=GeoGlobe.Util.getShadeColor(u,-40),c.fill(),c.beginPath(),c.moveTo(this.parts[f].cubes[k].vertexs[0][0],this.parts[f].cubes[k].vertexs[0][1]-this.parts[f].cubes[k].height*this.energy),c.lineTo(this.parts[f].cubes[k].vertexs[1][0],this.parts[f].cubes[k].vertexs[1][1]-this.parts[f].cubes[k].height*this.energy),c.lineTo(this.parts[f].cubes[k].vertexs[2][0],this.parts[f].cubes[k].vertexs[2][1]-this.parts[f].cubes[k].height*this.energy),c.lineTo(this.parts[f].cubes[k].vertexs[3][0],
this.parts[f].cubes[k].vertexs[3][1]-this.parts[f].cubes[k].height*this.energy),c.closePath(),c.fillStyle=GeoGlobe.Util.getShadeColor(u,-20),c.fill(),c.shadowColor=m.shadowColor,c.shadowBlur=m.shadowBlur,c.shadowOffsetX=m.shadowOffsetX,c.shadowOffsetY=m.shadowOffsetY,c.globalCompositeOperation="destination-over",c.beginPath(),c.moveTo(this.parts[f].cubes[k].vertexs[1][0],this.parts[f].cubes[k].vertexs[1][1]),c.lineTo(this.parts[f].cubes[k].vertexs[1][0],this.parts[f].cubes[k].vertexs[1][1]-this.parts[f].cubes[k].height*
this.energy),c.lineTo(this.parts[f].cubes[k].vertexs[2][0],this.parts[f].cubes[k].vertexs[2][1]-this.parts[f].cubes[k].height*this.energy),c.lineTo(this.parts[f].cubes[k].vertexs[2][0],this.parts[f].cubes[k].vertexs[2][1]),c.closePath(),c.fillStyle=u,c.fill(),c.beginPath(),c.moveTo(this.parts[f].cubes[k].vertexs[2][0],this.parts[f].cubes[k].vertexs[2][1]),c.lineTo(this.parts[f].cubes[k].vertexs[2][0],this.parts[f].cubes[k].vertexs[2][1]-this.parts[f].cubes[k].height*this.energy),c.lineTo(this.parts[f].cubes[k].vertexs[3][0],
this.parts[f].cubes[k].vertexs[3][1]-this.parts[f].cubes[k].height*this.energy),c.lineTo(this.parts[f].cubes[k].vertexs[3][0],this.parts[f].cubes[k].vertexs[3][1]),c.closePath(),c.fillStyle=GeoGlobe.Util.getShadeColor(u,-40),c.fill(),c.beginPath(),c.moveTo(this.parts[f].cubes[k].vertexs[0][0],this.parts[f].cubes[k].vertexs[0][1]),c.lineTo(this.parts[f].cubes[k].vertexs[1][0],this.parts[f].cubes[k].vertexs[1][1]),c.lineTo(this.parts[f].cubes[k].vertexs[2][0],this.parts[f].cubes[k].vertexs[2][1]),c.lineTo(this.parts[f].cubes[k].vertexs[3][0],
...
a.name;var d=this.tooltipDiv.getElementsByTagName("ul")[0];d.innerHTML="";for(var e=0;e<this._legend.vLegendLabels.length;e++)this._legend.itemStatuses[e]!==0&&(d.innerHTML+='<li style="line-height:20px;'+(e===a.vActive?"":"opacity:0.6;")+'"><div style="width: 12px;height: 12px;margin: 5px 5px 0 0;float: left;background-color: '+(this._legend.itemColors[e]?this._legend.itemColors[e]:this._rendererOptions.markPoint.itemStyle.color)+';"></div><span style="fonts-weight: 600;">'+this._legend.vLegendLabels[e]+
